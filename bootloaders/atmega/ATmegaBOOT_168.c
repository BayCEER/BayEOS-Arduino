...
/*                                                        */
/*                                                        */
/* 20170614: Added RTC-OSCCAL Calibration code            */       
/*           for BayEOS Boards with RTC Quarz             */
...
volatile long timer0_overflow_count;
ISR(TIMER0_OVF_vect) {
...
volatile uint8_t adjust_osccal_flag=1;
...
ISR(TIMER2_OVF_vect){
	if(adjust_osccal_flag){
		...
		if(adjust_osccal_flag>2){
			if((current_micros-last_micros)>(1009000L/128)){
				if(OSCCAL) OSCCAL--;
				else adjust_osccal_flag=0; //reached limit!
			}
			else if((current_micros-last_micros)<(991000L/128)){
