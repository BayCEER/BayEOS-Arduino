...
HardwareSerialPlus::HardwareSerialPlus(
...
{
	_rx_buffer=rxbuffer;
	_rx_buffer_size=rxsize;
	_tx_buffer=txbuffer;
	_tx_buffer_size=txsize;
...
HardwareSerialPlus::HardwareSerialPlus(
  ...
  volatile uint8_t *ucsra, volatile uint8_t *ucsrb,
  volatile uint8_t *ucsrc, volatile uint8_t *udr,
  unsigned char *rxbuffer, uint16_t rxsize,unsigned char *txbuffer, uint16_t txsize) : HardwareSerial(ubrrh,ubrrl,ucsra,ucsrb,ucsrc,udr)
{
	_rx_buffer=rxbuffer;
	_rx_buffer_size=rxsize;
	_tx_buffer=txbuffer;
...
void HardwareSerialPlus::_rx_complete_irq(void)
{
  if (bit_is_clear(*_ucsra, UPE0)) {
    // No Parity error, read byte and store it in the buffer if there is
    // room
    unsigned char c = *_udr;
    uint16_t i = (unsigned int)(_rx_buffer_head + 1) % _rx_buffer_size;
