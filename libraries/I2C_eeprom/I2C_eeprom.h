...
private:
    ...
    int _pageBlock(uint16_t address, const uint8_t* buffer, uint16_t length, bool incrBuffer);
    int _WriteBlock(uint16_t address, const uint8_t* buffer, uint8_t length);
