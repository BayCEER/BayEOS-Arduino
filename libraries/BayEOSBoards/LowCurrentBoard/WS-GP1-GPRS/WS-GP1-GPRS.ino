...
void setup(void) {
  ...
  delay(1000);
  //CHANGE CONFIG!!
  client.readConfigFromStringPGM(PSTR("132.180.112.55|80|gateway/frame/saveFlat|import|import|WS-GP1-GPRS|pinternet.interkom.de|||PIN|"));
  res=client.begin(9600);
...
void loop()
{
  ...
  if (ISSET_ACTION(0)) {
    ...
    wind_count = 0;
    interrupts();
    client.addChannelValue(tmp_float / (float)(myRTC.now().get()-last_measurement));
    last_measurement=myRTC.now().get();
#endif
  ...
  if (ISSET_ACTION(7)) {
    ...
    if(gprs_counter>GPRS_SEND_COUNT){
      ...
      bat=BAT_MULTIPLIER*analogRead(A0);
      //We got to many errors or buffer is empty
      if(tx_error>100 || ! myBuffer.available()){
        gprs_counter=0;
        gprs_status=0;
        tx_error=0;
        digitalWrite(GPRS_POWER_PIN,LOW);
