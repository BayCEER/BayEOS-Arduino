 ...
 * Channels
 * 1: CPU-Time in ms
 * 2: Battery Voltage in V
 * 3: Temp in °C
 * 4: ADC
 * 5: Water level in mm (T-Comp.)
 * 6: Intensity in mm/h (filtered)
 * 7: Precipitation sum in mm
*/
...
#ifdef BayEOSLOGGER_H
        values[3] += y_cur[current_i];
#else
        client.addChannelValue(y_cur[current_i],5);
#endif
        if (calc_mode >= 1)
        ...
        if (calc_mode == 2)
        {
           ...
           float max_evap = -1 * MAX_EVAP * exp(17.62 * temp0 / (243.12 + temp0)) / exp(17.62 * 20 / (243.12 + 20)) * SAMPLING_INT / 3600;
            ...
            if (diff < 0)
                diff = 0;
#ifdef BayEOSLOGGER_H
            values[4] += diff * 3600 / SAMPLING_INT;
            values[5] += rainsum;
#else
