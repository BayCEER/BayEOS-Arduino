...
// Function to use with Sleep-Mode
// Returns 0 on success
// While waiting the AVR sleeps with timer2
// timeoutcounter is default 30. This should be sufficient for timer2 settings of 128 ticks per second.
//
int8_t Sensirion::measureSleep(float *temp, float *humi,uint8_t timeoutcounter){
	  int8_t error = 0;
	  if(error = meas(0)) return error; //Temperature request
	  while(! measRdy(0)){
		  Sleep.sleep(TIMER2_ON, SLEEP_MODE_PWR_SAVE);
		  timeoutcounter--;
		  if(! timeoutcounter) return(S_Err_TO);
	  }
	  *temp = calcTemp(_meas[TEMP]);
	  if(error = meas(1)) return error; //Humidity request
	  while(! measRdy(1)){
		  Sleep.sleep(TIMER2_ON | TWI_ON,SLEEP_MODE_PWR_SAVE);
		  timeoutcounter--;
		  if(! timeoutcounter) return(S_Err_TO);
	  }
